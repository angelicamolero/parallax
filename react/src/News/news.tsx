...
const News = ({ news, isMobile, mode, blockClass, viewModeDesktop, gap, fr, title, useBackground }: NewsProps) => {
  ...
  return <div className={gclasses}>
    ...
    <div className={classes} style={{ gridTemplateColumns: "repeat(" + viewModeDesktop + ", " + fr + ")", gridGap: gap, display: isMobile ? 'block' : 'grid' }}>{
      news && news.length ?
        news.map((n, i) => {
          return mode == 'out' ? <NewsItem {...n} key={i} isMobile={isMobile} useBackground={useBackground}/> : <NewsItemInto {...n} key={i} isMobile={isMobile} useBackground={useBackground}/>
        }) : <React.Fragment />
    }</div>
  </div>
}
